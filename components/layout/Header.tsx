...
const navLinks = [
  ...
  { label: 'Our Clients & Projects', href: '/clients-and-projects' },
    ...
    <>
      <header
        ...
        style={{
          ...
          boxShadow: scrolled ? '0 2px 20px rgba(0,0,0,0.08)' : '0 1px 0 #E6E8F0',
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-4">
            ...
            <nav className="hidden xl:flex items-center flex-1 justify-center min-w-0">
              <div
                className="flex items-center gap-0.5 rounded-full px-1.5 py-1 whitespace-nowrap"
                style={{
                  background: isLight ? 'rgba(11,26,91,0.06)' : 'rgba(255,255,255,0.06)',
                  border: `1px solid ${isLight ? 'rgba(11,26,91,0.12)' : 'rgba(255,255,255,0.1)'}`,
                }}
              >
                <Link href="/" className="px-3.5 py-1.5 text-[13px] font-semibold text-white rounded-full transition-all" style={{ background: 'rgba(199,46,158,0.9)' }}>
                  Home
                </Link>

                {/* Services with fixed dropdown gap */}
                <div
                  className="relative"
                ...
                >
                  <Link
                    href="/services"
                    className="px-3.5 py-1.5 text-[13px] font-medium rounded-full transition-all flex items-center gap-1"
                    style={{
                      color: isLight ? '#0B1A5B' : 'rgba(255,255,255,0.8)',
                      background: servicesOpen ? (isLight ? 'rgba(11,26,91,0.08)' : 'rgba(255,255,255,0.1)') : 'transparent',
                    }}
                  >
                    Services
                  ...
                  {/* Dropdown with pt-2 bridge — no gap, no disappear */}
                  {servicesOpen && (
                    <div className="absolute top-full left-0 pt-2 z-50" onMouseEnter={openServices} onMouseLeave={closeServices}>
                      <div className="w-72 rounded-2xl shadow-2xl py-2" style={{ background: '#06103C', border: '1px solid rgba(255,255,255,0.1)' }}>
                        {services.map((s) => (
                          <Link
                            key={s.href}
                            href={s.href}
            ...
            <button
              ...
              className="xl:hidden relative z-[60] p-2 rounded-lg flex-shrink-0"
              aria-label="Toggle menu"
              style={{ color: isLight ? '#0B1A5B' : 'rgba(255,255,255,0.8)' }}
            >
              <div className="w-6 h-5 flex flex-col justify-between">
                <span className={`block h-0.5 rounded-full transition-all duration-300 origin-left ${isLight ? 'bg-[#0B1A5B]' : 'bg-white'} ${isOpen ? 'rotate-45 translate-y-px' : ''}`} />
                <span className={`block h-0.5 rounded-full transition-all duration-300 ${isLight ? 'bg-[#0B1A5B]' : 'bg-white'} ${isOpen ? 'opacity-0 scale-x-0' : ''}`} />
                <span className={`block h-0.5 rounded-full transition-all duration-300 origin-left ${isLight ? 'bg-[#0B1A5B]' : 'bg-white'} ${isOpen ? '-rotate-45 -translate-y-px' : ''}`} />
              </div>
            </button>
          </div>
          ...
          <div className="flex-1 space-y-1 overflow-y-auto px-4 py-5 sm:px-6 sm:py-6">
            {mobileNavLinks.map((link) => (
              <Link key={link.href} href={link.href} onClick={() => setIsOpen(false)}
                ...
                style={{ color: '#0B1A5B' }}>
                {link.label}
                <svg className="w-4 h-4 opacity-30" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: '#0B1A5B' }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </Link>
            ))}
            <div>
              <button onClick={() => setMobileServicesOpen(!mobileServicesOpen)}
                className="w-full flex items-center justify-between px-4 py-3.5 text-base font-medium rounded-2xl transition-all hover:bg-gray-50"
                style={{ color: '#0B1A5B' }}>
                Services
              ...
              <div className="overflow-hidden transition-all duration-300" style={{ maxHeight: mobileServicesOpen ? `${services.length * 48 + 56}px` : '0px' }}>
                <div className="ml-4 mt-1 space-y-0.5 pl-4" style={{ borderLeft: '2px solid rgba(168,34,138,0.3)' }}>
                  <Link href="/services" onClick={() => setIsOpen(false)} className="block py-2.5 text-sm font-semibold" style={{ color: '#0B1A5B' }}>All Services</Link>
                  {services.map((s) => (
                    <Link key={s.href} href={s.href} onClick={() => setIsOpen(false)} className="block py-2.5 text-sm" style={{ color: '#6B7280' }}>{s.name}</Link>
                  ))}
                </div>
              </div>
            </div>
