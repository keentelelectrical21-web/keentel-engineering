...
export default function RenewableSpecialtyPage({ config }: { config: RenewableSpecialtyConfig }) {
  return <><Header /><main>
    ...
    <section className="bg-white py-12 sm:py-16"><div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:gap-14 lg:px-8"><div><p className="mb-4 font-jost text-xs font-bold uppercase tracking-widest text-[#A8228A]">Our Approach</p><h2 className="mb-5 font-urbanist text-3xl font-black leading-tight text-[#06103C] sm:text-4xl">{config.approachTitle}</h2><p className="font-jost text-base leading-relaxed text-gray-600 sm:text-lg">{config.approachCopy}</p></div><img src={config.image} alt={config.imageAlt} className="h-72 w-full rounded-2xl object-cover shadow-xl sm:h-96" /></div></section>
    <ContactForm />
